...
part of "app_rating_bar.dart";

class _RatingBarIconTypes {
  final Widget full;
  final Widget half;
  final Widget empty;

  _RatingBarIconTypes({required this.full, required this.half, required this.empty});
}
    ...
    return SizedBox(
      ...
      child: enableMask
          ? Stack(
              ...
              children: [
                FittedBox(
                  child: _NoRatingWidget(size: size, alpha: alpha, unratedColor: unratedColor, child: child),
                ),
                FittedBox(
                  child: ClipRect(
...
class _HalfClipper extends CustomClipper<Rect> {
  ...
  _HalfClipper({this.rtlMode = false});
...
class _NoRatingWidget extends StatelessWidget {
  ...
  const _NoRatingWidget({this.size, this.child, this.alpha, this.enableMask = true, this.unratedColor = Colors.white});
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      width: size,
      child: FittedBox(
        child: enableMask ? _ColorMask(color: unratedColor.withAlpha(255 - alpha!), child: const Icon(Icons.star_border)) : child,
      ),
    );
  }
}
...
class _ColorMask extends StatelessWidget {
  ...
  const _ColorMask({this.color, this.child});
...
class RatingBar extends StatefulWidget {
  ...
  /// Customizes the Rating Bar item with [_RatingBarIconTypes].
  final _RatingBarIconTypes? ratingWidget;
  ...
  Widget _buildRating(BuildContext context, int index) {
    ...
    if (index >= _rating) {
      ...
      ratingWidget = SizedBox(
        ...
        child: FittedBox(child: widget.ratingWidget?.full ?? widget.itemBuilder!(context, index)),
      );
      iconRating += 1.0;
    }
