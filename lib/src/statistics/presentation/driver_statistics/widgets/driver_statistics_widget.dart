...
import '../driver_statistics_cubit.dart';
import '../utils/get_driver_statistics_subscription.dart';

part 'statistics_card.dart';

class DriverStatisticsWidget extends StatelessWidget {
  const DriverStatisticsWidget({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(create: (context) => injector<DriverStatisticsCubit>()..getStatistics(), child: const _StatisticsBody());
  }
}
...
class _StatisticsBodyState extends State<_StatisticsBody> {
  ...
  late final DriverStatisticsCubit _cubit;
  ...
  void initState() {
    super.initState();
    _cubit = context.read<DriverStatisticsCubit>();
  ...
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<DriverStatisticsCubit, DriverStatisticsState>(
      builder: (context, state) {
        if (state.getStatisticsState.isLoading) {
          ...
          return SizedBox(height: 50, child: AppFailWidget.mini(onRetry: () => context.read<DriverStatisticsCubit>().getStatistics()));
